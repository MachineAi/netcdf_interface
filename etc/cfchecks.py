...
#===============================================================================
#
#Modified by: Nicolai Holzer, University of Strasbourg (France)
#    Changes in program code marked within '==========' notation
#
...
#Changelog
#-------------------------------------------------------------------------------
#2010-12-21: v0.1.0 program adapted so that it can also be run from the data interface
#
#===============================================================================
#-------------------------------------------------------------
...
#Correct setting of library name
udunits=pDefaultSettings.udunitsLib#UDUNITS_LIB #CDLL("libudunits2.so.0.0.0") #Now global constant in interface_Settings.py

#Adaption of constants
import os
DIRECTORY=os.getcwd()+'/etc/' #"/home/gis/Documents/interface_v3a/src/etc/" #New constant
STANDARDNAME=DIRECTORY+"cf-standard-name-table.xml"
AREATYPES=DIRECTORY+"area-type-table.xml"
UDUNITS=str(pDefaultSettings.udunitsXml)#UDUNITS_XML #'/usr/share/xml/udunits/udunits2.xml'#Now global constant in interface_Settings.py#udunitsDat
...
def startCfChecksFromInterface(infile_):
    """
    Function to run the CfChecker out from another program.

    This function allows to start the cfchecks-program out of another function instead
    of using the main-function with related arguments. The program code was assembled
    by the use of code of other functions within this program.

    INPUT_PARAMETERS:
    infile      - name of NetCDF file with filename suffix

    COMMENTS:
    Other changes marked within '==========' notation in the program code
    """

    #code obtained and adapted from function 'def getargs(arglist)'
    #-------------------------------------------------------------------------------
    ...
    # set defaults
    udunits=UDUNITS #Uncomment the default setting from '' to new constant UDUNITS
    ...
    badc=None
    coards=None
    version=Versions[-1]#[-2] #For CF 1.3 and older no area type xml table is needed
    ...
    #defining parameter values to transfer from function 'def getargs(arglist)' to Main Program
    #-------------------------------------------------------------------------------

    #(badc,coards,uploader,useFileName,standardname.strip(),areatypes.strip(),udunits.strip(),version,args)
    #    --> (badc,coards,uploader,useFileName,standardName,areaTypes,udunitsDat,version,files)
    standardName = standardname.strip()
    areaTypes = areatypes.strip()
    udunitsDat = udunits.strip()
    files = infile_ #args


    #code obtained and adapted from Main Program
    #-------------------------------------------------------------------------------
    ...
    #Check file
    rc = inst.checker(files)
    #rc > 0 --> Number of errors that occured;
    #rc < 0 --> Number of warnings that occured (no errors occured)
    #rc == 0 --> No errors and warnings occured
    
    return rc #Return error status
...
class CFChecker:
  ...
  def checker(self, file):
    ...
    #Additional error message added.

    try:
        if not os.path.exists(file):
            raise Exception("Error in CfCheck: File '" + str(file) + "' does not exist in current directory '" + str(os.getcwd()) + "'.")
        else:
...
##                    if len(varData) == 1:
  ...
  #------------------------------
    ...
    if self.f.attributes.has_key('Conventions'):
        ...
        if conventions not in CFVersions:
            ...
            if conventions == 'CF-1.4, epic-insitu-1.0':
                print "WARNING (interface): Error message disabled for convention statement 'CF-1.4, epic-insitu-1.0' that was found in the 'Conventions' attribute (Ok for Dapper In-situ Data Conventions)."
            else:
  ...
  #--------------------------
    ...
    if not nameSyntax.match(name):
        ...
        #Permit variable name ID since it is necessary for DChart dapperload program
        if name in ID:
            print "WARNING (interface): Error/Warning messages desactivated for variable name '" + str(ID) + "'. This is not a valid CF variable name (Ok for Dapper In-situ Data Conventions)!"
            return 1
        else:
      ...
      if var.attributes.has_key('units') and var.attributes['units'] != '':
          ...
          if var.id in allCoordVars:
              ...
              if not hasattr(var,'flag_values'):
                  ...
                  #Permit variable name ID since it is necessary for DChart dapperload program
                  if varName in ID:
                      print "WARNING (interface): Error/Warning messages desactivated for variable name '" + str(ID) + "'. A units attribute is missing (Ok for Dapper In-situ Data Conventions)!"
...
def getargs(arglist):
    ...
    #Uncomment the default setting from '' to new constant UDUNITS
    udunits=UDUNITS #''
